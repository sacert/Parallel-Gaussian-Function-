...
void computeGauss(int begin, int finish, int task_id)
{
    int i, j, k;
    double pivotval;
    
    if(task_num > 1) {
        begin = (nsize * (task_id -1 )) /task_num + 1;
        finish = (nsize * (task_id +1)) / task_num;
    }
    ...
    for (i = 0; i < nsize; i++) {
        ...
        barrier(task_num);
        if(task_id != (task_num -1) && finish != nsize) {
            finish = finish + 1;
        }
        if(task_id !=0 || task_num == 1) {
            /* Factorize the rest of the matrix. */
            for(j =i +begin; j < finish; j++) {
                pivotval = matrix[j][i];
                matrix[j][i] = 0.0;
                for (k = i + 1; k < nsize; k++) {
...
work_thread (void *lp)
{
    ...
    computeGauss(begin, end, task_id);
